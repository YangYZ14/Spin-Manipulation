...
class AWG:
    def __init__(self, ip: str = None, sample_rate: int = 9e9):
        # 初始化，设置IP地址，采样率，调用connect函数
        self.ip = ip
    ...
    def connect(self):
        # 连接仪器，设置IP地址，输出产品型号
        assert self.ip is not None, "IP address is required to connect to the AWG device"
        dev = 'TCPIP0::' + self.ip + '::5025::SOCKET'
        self.awg = self.resource_manager.open_resource(dev)
    ...
    def set_sample_rate(self):
        # 设置采样率
        self.awg.write(':FREQ:RAST {}'.format(self.sample_rate))
    ...
    # Example usage
    # awg = AWG(ip='192.168.0.1', sample_rate=1e9)
    # awg.reset()
    # awg.set_sample_rate()
    # awg.set_waveform_format()
    # awg.set_DAC_mode()
