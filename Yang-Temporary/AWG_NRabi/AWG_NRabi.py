...
class Interface:
    def __init__(self):
        #导入UI
        self.ui = QUiLoader().load('AWG_NRabi.ui')
    ...
    def start(self):
        ...
        self.MWPower = self.ui.doubleSpinBox_4.value() #暂时还没有用到
        ...
        self.All_sequence_on_1 = []
        self.All_sequence_on_2 = []
        self.All_sequence_off_1 = []
        self.All_sequence_off_2 = []
        print(self.timelist)

        for t in self.timelist:
            #On模式（加微波pi）通道一输出段：包含控制输出微波、AOM、NI
            self.odmr_sequence1_1 = [['step1', 2, 100000],
                                ['step2', 1, self.MW_t_pi],
                                ['step3', 0, 5000],
                                ['step4', 1, self.MW_t_pi],
                                ['step5', 6, 50000]]
            # On模式（加微波pi）通道二输出段：仅包含控制输出射频
            self.odmr_sequence1_2 = [['step1', 0, 100000],
                                ['step2', 0, self.MW_t_pi],
                                ['step3', 1, t * 1000],
                                ['step6', 0, 5000 - t * 1000],
                                ['step4', 0, self.MW_t_pi],
                                ['step5', 0, 50000]]
            ...
            self.odmr_sequence2_2 = [['step1', 0, 100000],
                                ...
                                ['step5', 0, 50000]]
            self.All_sequence_on_1.append(self.odmr_sequence1_1)
            self.All_sequence_on_2.append(self.odmr_sequence1_2)
            self.All_sequence_off_1.append(self.odmr_sequence2_1)
            self.All_sequence_off_2.append(self.odmr_sequence2_2)
    ...
    def loop(self):
        while self.running:
            ...
            self.On_seglen1_1, self.On_Channel1_Segment, self.On_Marker_Segment1, self.On_Marker_Segment1_1, self.On_Marker_Segment1_2, self.Squence_time_all = self.AWG_TASK.sequence_set(self.All_sequence_on_1[self.i], self.MWFreq, self.sample_rate)
            self.On_seglen1_2, self.On_Channel2_Segment, self.On_Marker_Segment2, self.On_Marker_Segment1_1, self.On_Marker_Segment1_2, self.Squence_time_all = self.AWG_TASK.sequence_set(self.All_sequence_on_2[self.i], self.RFFreq, self.sample_rate)
