...
class AWG:
    def __init__(self,IP=None, SampleRate=9e9):
        """
        Initializes the AWG object.

        Args:
        - IP (str): The IP address of the AWG device.
        - SampleRate (float): The sample rate of the AWG signal.
        """
        self.IP = IP
    ...
    def connect_AWG(self):
        """
        Connects to the AWG device.

        Returns:
        - awg (visa.ResourceManager): The AWG device object.
        """
        resourceManager = visa.ResourceManager()
        dev = 'TCPIP0::' + self.IP + '::5025::SOCKET'
        ...
        print('连接成功')
        return self.awg # 这个能输出吗？将前面的query return或许更好
    ...
    #     """
    #     Sets the instrument to continuous mode. 感觉没必要？
    #     """
    ...
    def set_waveform_width(self):
        """
        Sets the waveform width.
        """
        if resp == 'U8':
            self.wpt_width = 8
        elif resp == 'U16':
    ...
    def set_sample_rate(self):
        """
        Read and Set the sample rate.
        """
        resp = self.awg.query(':FREQ?')
        print("Sample Rate: {0}".format(resp))
        self.awg.write(':FREQ:RAST ' + str(self.SampleRate))
