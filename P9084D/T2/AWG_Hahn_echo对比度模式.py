...
class Interface:
	...
	def start(self):
		...
		self.All_sequence = []
		self.t_pi = self.ui.doubleSpinBox_3.value()
		self.t_pi_2 = self.ui.doubleSpinBox_6
		print(self.timelist)
		...
		for t in self.timelist:
			...
			self.sequence = [['step1', 2, 3 * 1000],
							 ['step2', 0, 1 * 1000],
							 ['stpe3', 1, self.t_pi_2 * 1000],
							 ['step4', 0, t/2 * 1000],
							 ['stpe5', 1, self.t_pi * 1000],
							 ['step6', 0, t/2 * 1000],
							 ['stpe7', 1, self.t_pi_2 * 1000],
							 ['step8', 2, 0.31 * 1000],
							 ['step9', 6, 0.55 * 1000],

							 ['step11', 2, 3 * 1000],
							 ['step12', 0, 1 * 1000],
							 ['step13', 0, (self.t_pi + 2*self.t_pi_2 + t) * 1000],
							 ['step14', 2, 0.31 * 1000],
							 ['stpe15', 6, 0.55 * 1000],
							 ['step16', 2, 0.02 * 1000]]
		...
		self.t2_contrast1 = np.zeros(self.N + 1)
		self.t2_contrast2 = np.zeros(self.N + 1)
	...
	def loop(self):
		while self.running:
			if self.i == self.N + 1:
				self.t2_contrast2 = (self.t2_contrast2*(self.cycle)+self.t2_contrast1)/(self.cycle + 1)
				self.t2_contrast1 = np.zeros(self.N + 1)
				self.i = 0
				self.cycle += 1
			...
			self.t2_contrast1[self.i] = (self.On_counts - self.Off_counts) / self.Off_counts
			if self.t2_contrast1[self.i] > 0:
				self.t2_contrast1[self.i] = -self.t2_contrast1[self.i]
			print('t2 contrast:')
			print(self.t2_contrast1)
			self.i += 1

	def update(self):
		self.ui.lineEdit.setText(str(self.cycle))
		self.p1.plot(self.x,self.t2_contrast1,pen='g',clear=True)
		self.p2.plot(self.x,self.t2_contrast2,pen='g',clear=True)
