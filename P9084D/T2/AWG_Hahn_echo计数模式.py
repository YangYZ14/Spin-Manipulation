...
#存在人为纠正On、Off数据顺序问题
#对于Hahn_echo测量序列的描述
#计数部分   ①激光极化→②极化后驰豫→③开始施加共振微波→④共振驰豫→⑤激光自旋读出→⑥继续自旋读出并NI计数→⑦加上一段激光极化（为何）
...
class Interface:
	...
	def start(self):
		...
		self.t2_counts1 = np.zeros(self.N + 1)
		self.t2_counts2 = np.zeros(self.N + 1)
	...
	def loop(self):
		while self.running:
			if self.i == self.N + 1:
				self.t2_counts2 = (self.t2_counts2*(self.cycle)+self.t2_counts1)/(self.cycle + 1)
				self.t2_counts1 = np.zeros(self.N + 1)
				self.i = 0
				self.cycle += 1
			...
			self.t2_counts1 = sum(self.data)
			print('t1 counts:')
			print(self.t2_counts1)
			self.i += 1


	def update(self):
		self.ui.lineEdit.setText(str(self.cycle))
		self.p1.plot(self.x,self.t2_counts1,pen='g',clear=True)
		self.p2.plot(self.x,self.t2_counts2,pen='g',clear=True)
