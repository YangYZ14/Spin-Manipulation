...
class AWG:
    def __init__(self,IP=None):
        ...
        self.resp = self.awg.query('*IDN?')
        print('仪器型号：' + self.resp)
        print('连接成功')


        
        self.sample_rate_dac = int(9e9)
        self.awg.timeout = 30000

        # 重置仪器
        self.awg.write('*CLS; *RST')
        self.awg.write(':FREQ:RAST 9e9')
        ...
        self.resp = self.awg.query(':SYST:INF:DAC?')
        self.dac_mode = self.resp
        print('DAC mode {0} '.format(self.resp))
        if self.dac_mode == 'M0':
            self.max_dac = 65535
    ...
    def download_channel_segment(self,seglen = 640000, Channel1_Segment = np.array([]).astype(np.uint8), power = 1.3,segnum = 1):
        ...
        #定义第几段和这个段对长度
        self.awg.write(':TRAC:DEF {0},{1}'.format(str(self.segnum), str(self.seglen)))
        # 选择定义的segment
        self.awg.write(':TRAC:SEL {0}'.format(str(self.segnum)))
        # 在写入二进制数据之前增加超时时间
        self.awg.write_binary_values(':TRAC:DATA 0', self.Channel1_Segment, datatype='B')
        #设置channel1输出的幅值
    ...
    def download_marker_segment(self, Marker_Segment = np.array([]).astype(np.uint8), marker1_ptop = 0, marker2_ptop = 0, marker1_offs = 0, marker2_offs = 0, segnum = 1):
        ...
        self.awg.write_binary_values(':MARK:DATA 0', self.Marker_Segment, datatype='B')
    ...
    def sequence_task(self, loop_num = 100, segnb = 1):
        ...
        self.awg.write(':TASK:COMP:LENG 1')
        self.awg.write(':TASK:COMP:SEL 1')
        self.awg.write(':TASK:COMP:SEGM 1')
        self.awg.write(':TASK:COMP:LOOP 10000')
        self.awg.write(':TASK:COMP:DEF:IDLE:LEV 128')
        # self.awg.write(':TASK:COMP:ENAB CPU')
        # self.awg.write(':TASK:COMP:JUMP EVEN')
        # self.awg.write(':TASK:COMP:NEXT1 1')
        self.awg.write(':TASK:COMP:WRITE')
        self.awg.write(':FUNC:MODE TASK')
        resp = self.awg.query(':SYST:ERR?')
        resp = resp.rstrip()
        if not resp.startswith('0'):
            print('ERROR: "{0}" after set sequence binary values'.format(resp))
        else:
            print('没有报错')
    ...
    def start_sequence(self):
        ...
        resp = resp.rstrip()
        if not resp.startswith('0'):
            print('ERROR: "{0}" after start task binary values'.format(resp))
        else:
            print('没有报错')
        print('运行到这了')
        self.awg.write(':MARK:SEL 1')
        self.awg.write(':MARK ON')
        self.awg.write(':MARK:SEL 2')
        self.awg.write(':MARK ON')
        self.awg.write('OUTP ON')
        time.sleep(1)
    ...
    def stop_sequence(self):
        ...
        resp = resp.rstrip()
        if not resp.startswith('0'):
            print('ERROR: "{0}" after stop sequence'.format(resp))
        else:
            print('没有报错')
    ...
    def disconnect_awg(self,CLEAR_TEST = True, num_channels = 2):
        if CLEAR_TEST:
            cmd = ':INST:CHAN 1'
            self.awg.write(cmd, self.paranoia_level)
            cmd = ':TRAC:ZERO:ALL'
            self.awg.write(cmd, self.paranoia_level)
        chanlist = [1]
        imarker = 2
