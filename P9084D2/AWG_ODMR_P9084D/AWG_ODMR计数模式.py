...
class Interface:
    def __init__(self):
        ...
        self.p1.setLabel('left', text='Counyer', color='#ffffff')
        ...
        self.p2.setLabel('left', text='Counter', color='#ffffff')
        ...
        self.odmr_sequence = [['step1', 0, 30000],
                             ['step2', 7,100000]]
        #初始化NI，预设值
        self.singlecys = 10000
        self.loop_num = 0
    ...
    def start(self):
        ...
        self.counts_1 = np.zeros(self.N + 1)  #存储单次循环MW_list时的数据
        self.counts_2 = np.zeros(self.N + 1)   #存储多次循环MW_list时的平均数据
    ...
    def loop(self):
    	while self.running:
            if self.i == self.N + 1:  #初始i等于1
                self.counts_2 = (self.counts_2*self.cycle+self.counts_1)/(self.cycle+1)  #平均效应
                self.counts_1 = np.zeros(self.N + 1)  #单圈
                self.i = 0
                self.cycle += 1  #改变圈数
            ...
            self.task_num = 1
            self.seglen, self.Channel1_Segment, self.Marker_Segment = self.AWG_TASK.sequence_set(self.odmr_sequence,self.MW_list[self.i],self.sample_rate)
            ...
            self.counts_1[self.i] = sum(self.data) 
            print('data:')
            print(self.counts_1)
            self.i += 1  #i开始迭代
    ...
    def update(self):
        ...
        self.p1.plot(self.MW_list, self.counts_1, pen='g', clear=True)
        self.p2.plot(self.MW_list, self.counts_2,pen='r',clear=True)
